(function () {
  ...
  // Legionnaire Insights v8.2.1: one event-driven runtime, no gameplay polling,
  ...
  // option card and walk only its bounded parent chain to the owning component.
  // React alternates fibers between commits, so inspect the corresponding
  // alternate at each level and accept only props matching the rendered card.
  function localDecisionProps(card, cardIndex, seed, step) {
    ...
    const decisionPrefix = `${seed}-${step}-`;
    for (const pageCard of candidates) {
      if (!pageCard) continue;
      const key = Object.keys(pageCard).find((name) => name.startsWith('__reactFiber$'));
      let fiber = key ? pageCard[key] : null;
      for (let depth = 0; fiber && depth < 8; depth++, fiber = fiber.return) {
        for (const branch of fiber.alternate ? [fiber, fiber.alternate] : [fiber]) {
          const props = branch.memoizedProps;
          if (props && String(props.decision?.id || '').startsWith(decisionPrefix) && propsMatchVisibleCard(card, props)) return props;
        }
      }
    }
    return null;
  ...
  function decisionStep(save) {
    return save && Array.isArray(save.choices) ? save.choices.length : null;
  }
  ...
  function decoratePredictions() {
    ...
    const record = activeSaveRecord();
    if (!record) { clearPredictions(); return 0; }
    const step = decisionStep(record.save);
    if (!Number.isInteger(step)) { clearPredictions(); return 0; }
    let decorated = 0;
    ...
    for (let cardIndex = 0; cardIndex < cards.length; cardIndex++) {
      ...
      let props;
      try { props = localDecisionProps(card, cardIndex, record.save.seed, step); } catch (e) { props = null; }
      const prediction = props ? predictedOutcome(record.save.seed, step, props.option) : null;
      const pills = [...card.querySelectorAll('.pill')];
