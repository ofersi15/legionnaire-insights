(function () {
  ...
  // Legionnaire Insights v8.0.1: one event-driven runtime, no gameplay polling,
  // no React scans, and no localStorage fingerprinting.
  ...
  function rebuildClubMaps(items, source) {
    ...
    for (const item of items || []) {
      ...
      for (const alias of [item.name, item.shortName, ...(item.aliases || [])]) {
          ...
          try {
            ...
            for (const row of parsed) {
              ...
              if (Array.isArray(row) && typeof row[0] === 'string') {
                item = { id: row[0], name: row[1], shortName: row[2], league: row[4], ovr: Number(row[6]) };
              } else if (row && typeof row === 'object' && typeof row.id === 'string') {
                item = { id: row.id, name: row.name, shortName: row.shortName, league: row.league, ovr: Number(row.baseOverall) };
              }
  ...
  window.addEventListener('resize', () => {
    ...
    if (hud) { const r = hud.getBoundingClientRect(), p = clamp(hud, r.left, r.top); hud.style.left = `${p.left}px`; hud.style.top = `${p.top}px`; }
  });

  for (const delay of [100, 300, 700, 1400, 2400]) setTimeout(() => { syncHud(); decorateClubs(); }, delay);
