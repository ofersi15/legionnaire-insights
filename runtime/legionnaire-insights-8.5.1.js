(function () {
  ...
  // Legionnaire Insights v8.5.1: player and coach careers in one event-driven runtime,
  ...
  function clearPredictions() {
    document.querySelectorAll('.decision .option').forEach(clearPrediction);
  ...
  function indexManagerEventOptions(source) {
    ...
    while ((eventMatch = eventPattern.exec(source))) {
      ...
      while ((optionMatch = optionPattern.exec(optionsSource))) {
        ...
        candidates.push({ id: `${eventKey}-${optionKey}`, eventKey, title, sport: /_BB$/.test(eventKey) ? 'basketball' : 'football', outcomes });
  ...
  function managerEventOptionFromCard(card, bands) {
    const label = norm(card.querySelector('.option__name,.spend__label')?.textContent);
    ...
    const matches = (managerEventOptionsByLabel.get(label) || []).filter((candidate) =>
  ...
  function inferredManagerStep(save) {
    ...
    for (let index = choices.length - 1; index >= 0; index--) {
      const id = String(choices[index] || '');
      if (!/^mgr-(?:train|stay|switch|continue|event|shop|scout|sign|national|campaign|retire)-/.test(id)) continue;
  ...
  function decorateManagerDomPredictions(record) {
    ...
    for (const card of document.querySelectorAll('.decision button')) {
      if (!visible(card) || card.closest(`[${UI_ATTR}]`) || card.querySelector(`[${PREDICTION_MARKER_ATTR}]`)) continue;
  ...
  function localDecisionProps(card, cardIndex, records) {
    ...
    const bridgedCard = pageDocument?.querySelectorAll('.decision .option')[cardIndex];
  ...
  function decoratePredictions() {
    ...
    let decorated = 0;
    const cards = document.querySelectorAll('.decision .option');
