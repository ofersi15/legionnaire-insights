(function () {
  ...
  // Legionnaire Insights v8.2.0: one event-driven runtime, no gameplay polling,
  ...
  function localDecisionProps(card, cardIndex) {
    ...
    for (const pageCard of candidates) {
      ...
      for (let depth = 0; fiber && depth < 8; depth++, fiber = fiber.return) {
        ...
        if (props && props.decision && props.option && Array.isArray(props.option.outcomes)) return props;
      }
    }
    return null;
  }
  ...
  function decoratePredictions() {
    ...
    for (let cardIndex = 0; cardIndex < cards.length; cardIndex++) {
      ...
      let props;
      try { props = localDecisionProps(card, cardIndex); } catch (e) { props = null; }
      const step = props && decisionStep(record.save);
      const prediction = Number.isInteger(step) ? predictedOutcome(record.save.seed, step, props.option) : null;
