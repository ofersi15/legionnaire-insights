(function () {
  ...
  // Legionnaire Insights v8: one runtime, no gameplay polling, no React scans.
  // Sync is compatible with the existing v7 device-snapshot format but only
  // runs on startup/resume (pull), retirement (push), or explicit user action.
  ...
  const CLUB_CACHE_KEY = 'legionnaire-insights:club-cache-v2';
  ...
  function activeSaveKey() { return PER_SPORT_KEYS[sport()].save; }
  ...
  function activeSave() { return readJson(activeSaveKey(), null); }
  ...
  function currentCreation() {
    ...
    try { return computeCreation(save.seed); } catch (e) { return null; }
  }
  ...
  function findVisibleOvrTile() {
    return measure('ovr-detect', () => {
      const labels = document.querySelectorAll('div,span,p');
      for (const label of labels) {
        if (!visible(label) || norm(label.textContent).toUpperCase() !== 'OVR') continue;
        let node = label.parentElement;
        for (let depth = 0; node && depth < 5 && node !== document.body; depth++, node = node.parentElement) {
          if (!visible(node)) continue;
          const r = node.getBoundingClientRect();
          if (r.width < 60 || r.width > 200 || r.height < 55 || r.height > 190) continue;
          const numbers = norm(node.textContent).match(/\b\d{2,3}\b/g) || [];
          if (numbers.length) return { element: node, overall: Number(numbers[numbers.length - 1]) };
        }
      }
      return null;
    });
  ...
  function careerContext() {
    const save = activeSave();
    if (!save || !save.seed) return { active: false, save: null, creation: null, overall: null };
    const tile = findVisibleOvrTile();
    if (!tile) return { active: false, save, creation: null, overall: null };
    return { active: true, save, creation: computeCreation(save.seed), overall: tile.overall };
  }
  ...
  function ensureStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      #${HUD_ID}{position:fixed;z-index:1000000;display:inline-flex;align-items:center;gap:5px;min-height:31px;padding:5px 9px;border:1px solid rgba(74,222,128,.52);border-radius:999px;background:rgba(7,10,14,.92);color:#e5e7eb;box-shadow:0 4px 14px rgba(0,0,0,.28);backdrop-filter:blur(8px);font:850 10px/1 system-ui,-apple-system,Segoe UI,sans-serif;direction:ltr;touch-action:none;user-select:none;-webkit-user-select:none;cursor:grab}
      ...
      .li-v8-card{border:1px solid #303641;border-radius:12px;background:#11151b;padding:10px;margin:8px 0}.li-v8-kv{display:grid;grid-template-columns:auto 1fr;gap:5px 10px;align-items:baseline}.li-v8-kv span{color:#94a3b8;font-size:11px}.li-v8-kv strong{direction:ltr;text-align:left}.li-v8-highlight{color:#facc15}.li-v8-green{color:#86efac}.li-v8-muted{color:#94a3b8;font-size:11px}.li-v8-status{min-height:20px;margin-top:9px;color:#94a3b8;font-size:11px}
      ...
      .li-v8-agent{border-bottom:1px solid #252b34;padding:9px 0}.li-v8-agent:last-child{border-bottom:0}.li-v8-agent-name{font-weight:900}.li-v8-agent-meta{color:#aeb6c3;font-size:11px;margin-top:3px}.li-v8-agent-clubs{color:#86efac;font-size:11px;margin-top:4px}
      .li-v8-debug{direction:ltr;text-align:left;font:11px/1.55 ui-monospace,SFMono-Regular,Consolas,monospace;white-space:pre-wrap}
      @media(min-width:700px){.li-v8-overlay{align-items:center}}@media(max-width:640px){[${BADGE_ATTR}]{font-size:8px!important;padding:2px 5px!important}.li-v8-fields{grid-template-columns:1fr 1fr}.li-v8-fields .li-v8-field:last-child{grid-column:1/-1}}
    `;
  ...
  function writeHudPos(left, top) {
    try { localStorage.setItem(HUD_POS_KEY, JSON.stringify({ left: Math.round(left), top: Math.round(top) })); } catch (e) {}
  }
  ...
  function syncHud() {
    ...
    const state = !enabled() ? 'hidden' : context.active ? `pot:${context.creation.potential}` : 'plain';
  ...
  function loadCachedClubs() {
    try {
      ...
      for (const item of cache.items) {
        ...
        clubByName.set(norm(item.name), item);
        if (item.id) clubById.set(item.id, item);
      }
    } catch (e) {}
  }
        ...
        while (true) {
          ...
          try {
            ...
            for (const row of parsed) {
              let item = null;
              if (Array.isArray(row) && typeof row[0] === 'string') item = { id: row[0], name: row[1], league: row[4], tier: row[5], ovr: Number(row[6]) };
              else if (row && typeof row === 'object' && typeof row.id === 'string') item = { id: row.id, name: row.name, league: row.league, tier: row.tier, ovr: Number(row.baseOverall) };
              if (item && item.name && Number.isFinite(item.ovr)) itemsById.set(item.id, item);
            }
          } catch (e) {}
        }
        if (!itemsById.size) return clubByName;
        const items = [...itemsById.values()];
        clubByName = new Map(items.map((x) => [norm(x.name), x]));
        clubById = new Map(items.map((x) => [x.id, x]));
  ...
  function decorateClubs() {
    return measure('club-dom-scan', () => {
      ...
      for (const { card, club } of found) {
        ...
        if (club.ovr === best) card.classList.add(BEST_CLASS);
        const badge = document.createElement('span');
        badge.setAttribute(BADGE_ATTR, '1');
        badge.setAttribute(UI_ATTR, 'badge');
        badge.textContent = `OVR ${club.ovr}`;
  ...
  function scheduleUiBurst() {
    ...
    for (const delay of [80, 350]) {
      uiTimers.push(setTimeout(() => { syncHud(); decorateClubs(); }, delay));
    }
  }
  ...
  function sheet(title, subtitle, bodyHtml) {
    ...
    overlay.addEventListener('click', (event) => {
      if (event.target === overlay || event.target.closest('[data-close]')) closeSheet();
    });
  ...
  function openMainSheet() {
    ...
    const overlay = sheet('Legionnaire Insights', subtitle,
      `<div class="li-v8-grid"><button class="li-v8-btn" data-action="details">פרטים</button><button class="li-v8-btn" data-primary="1" data-action="seed">מצא סיד</button><button class="li-v8-btn" data-action="agents">סוכנים</button><button class="li-v8-btn" data-action="sync">Sync / Settings</button></div>`);
    overlay.addEventListener('click', (event) => {
  ...
  function openDetails() {
    ...
    if (!c.active) {
      sheet('פרטים', 'אין קריירה פעילה במסך הנוכחי', '<div class="li-v8-card li-v8-muted">ה־POT מוצג רק כשמסך הקריירה עצמו פתוח. אפשר להשתמש ב־Seed Finder או ב־Sync גם מכאן.</div>');
      return;
    }
    ...
    sheet('פרטי שחקן', 'מידע מחושב מה־save וה־UI בלבד — בלי סריקת React',
  ...
  // ---------- Sync merge primitives (compatible with v7 snapshots) ----------
  ...
  function mergeMapKey(key, remoteLedger) {
    ...
    for (const [leaf, devMap] of Object.entries(remoteLedger || {})) {
      ...
      for (const [devId, val] of Object.entries(devMap || {})) {
        if (devId !== myId) leafEntry[devId] = Math.max(leafEntry[devId] || 0, Number(val) || 0);
      }
      entry.ledger[leaf] = leafEntry;
    }
    const totals = {};
  ...
  function exportPayload() {
    return measure('sync-export-build', () => {
      ...
      for (const key of Object.keys(localStorage)) {
        if (key.startsWith('maslul-kariera') && !skip.has(key)) data[key] = localStorage.getItem(key);
      }
  ...
  function importPayload(payload) {
    ...
    for (const [key, remoteLedger] of Object.entries((payload.ledgers && payload.ledgers.numbers) || {})) {
      ...
      if (localStorage.getItem(key) !== before) changed = true;
    }
    ...
    for (const [key, sourceRaw] of Object.entries(payload.data || {})) {
      ...
      if (UNION_ARRAY_KEYS.has(key)) {
        const merged = unionBySeed(targetRaw ? JSON.parse(targetRaw) : [], sourceRaw ? JSON.parse(sourceRaw) : []);
  ...
  function rawRequest(url) {
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        method: 'GET', url, headers: { 'Cache-Control': 'no-cache, no-store', Pragma: 'no-cache' }, timeout: 30000,
        ...
        onerror: () => reject(new Error('Network error fetching snapshot')),
        ontimeout: () => reject(new Error('Snapshot request timed out')),
      });
    });
  }
  ...
  function getStoredToken() {
    ...
    if (!token) {
      try {
        token = localStorage.getItem(LEGACY_GH_TOKEN_KEY) || '';
        if (token) { GM_setValue(GH_TOKEN_GM_KEY, token); localStorage.removeItem(LEGACY_GH_TOKEN_KEY); }
      } catch (e) {}
    }
    return token;
  }
  ...
  async function getDeviceSnapshots(gist) {
    ...
    for (const [filename, file] of Object.entries((gist && gist.files) || {})) {
      ...
      try {
        ...
        if (!content) continue;
        result.push({ filename, ...(await unpackSnapshot(content)) });
  ...
  async function pullLegacyPayload() {
    ...
    for (const filename of Object.values(LEGACY_GIST_FILES)) {
      try { chunks[filename] = await rawRequest(bust(`https://gist.githubusercontent.com/${GIST_OWNER}/${GIST_ID}/raw/${filename}`)); }
      catch (e) { chunks[filename] = null; }
    }
    ...
    return {
      ...
      ts: meta.ts || Date.now(),
      ledgers: {
        numbers: safeObj(chunks[LEGACY_GIST_FILES.numbers]),
        maps: {
          [PER_SPORT_KEYS.football.collection]: safeObj(chunks[LEGACY_GIST_FILES.mapFootball]),
          [PER_SPORT_KEYS.basketball.collection]: safeObj(chunks[LEGACY_GIST_FILES.mapBasketball]),
        },
      },
      data,
    };
  ...
  async function pullAndMerge(opts = {}) {
    ...
    return measureAsync('sync-pull', async () => {
      ...
      let changed = false;
      if (snapshots.length) {
        for (const snapshot of snapshots) if (importPayload(snapshot.payload)) changed = true;
      } else {
        const legacy = await pullLegacyPayload();
        if (legacy && importPayload(legacy)) changed = true;
      }
  ...
  async function manualSync(statusEl) {
    ...
    syncInFlight = (async () => {
      try {
        ...
        const pulled = await pullAndMerge({ manual: true });
        ...
        if (pulled.changed) {
          ...
          button.className = 'li-v8-btn'; button.textContent = 'טען מחדש כדי לראות שינויים'; button.style.marginTop = '8px';
          button.addEventListener('click', () => location.reload());
          statusEl.appendChild(document.createElement('br')); statusEl.appendChild(button);
        }
  ...
  async function exportManual(status) {
    try {
      ...
      status.textContent = ok ? `הועתק ללוח · ${(text.length / 1024).toFixed(1)}KB` : 'לא הצלחתי להעתיק אוטומטית.';
      if (!ok) prompt('Copy this sync payload:', text);
  ...
  function importManual(status) {
    ...
    if (!text) return;
    try {
      const changed = importPayload(JSON.parse(text));
      status.textContent = changed ? 'Import הושלם. טען מחדש את המשחק.' : 'Import הושלם ללא שינויים.';
  ...
  function checkUpdate(force = false) {
    ...
    return new Promise((resolve) => {
      GM_xmlhttpRequest({
        method: 'GET', url: `${SCRIPT_UPDATE_URL}?v=${Date.now()}`, timeout: 15000,
        onload: (res) => {
          const match = res.status >= 200 && res.status < 300 ? res.responseText.match(/^\/\/ @version\s+([^\s]+)$/m) : null;
          if (match) { latestScriptVersion = match[1]; GM_setValue(UPDATE_LATEST_GM_KEY, latestScriptVersion); GM_setValue(UPDATE_LAST_CHECK_GM_KEY, Date.now()); }
          resolve(latestScriptVersion);
        },
  ...
  function formatTime(value) {
    ...
    return n ? new Date(n).toLocaleString() : 'אף פעם';
  }

  function debugReport() {
    const lines = [`Legionnaire Insights ${VERSION}`, `viewport ${innerWidth}x${innerHeight} DPR ${devicePixelRatio}`, ''];
  ...
  function openSyncSettings() {
    ...
    overlay.addEventListener('click', async (event) => {
      ...
      if (event.target.closest('[data-update]')) {
        ...
        status.textContent = latestScriptVersion && compareVersions(latestScriptVersion, currentVersion()) > 0 ? `קיימת גרסה ${latestScriptVersion}` : `מעודכן · ${currentVersion()}`;
      }
      if (event.target.closest('[data-debug]')) {
        const body = overlay.querySelector('[data-debug-body]');
        const show = body.style.display === 'none';
        body.style.display = show ? 'block' : 'none';
        body.textContent = debugReport();
  ...
  function completionState() {
    return {
      ...
      active: !!localStorage.getItem(activeSaveKey()),
    };
  }
  function retirementText(text) {
    const t = norm(text);
    return /פרישה|לפרוש|פרוש|סיום קריירה/.test(t);
  }
  ...
  async function startupPull() {
    ...
    try {
      ...
      if (result.changed && sessionStorage.getItem('legionnaire-insights:v8:reloaded') !== '1') {
        sessionStorage.setItem('legionnaire-insights:v8:reloaded', '1');
        location.reload();
      }
  ...
  async function resumePullIfDue() {
    ...
    try {
      const result = await pullAndMerge();
      if (result.changed) location.reload();
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    syncHud(); decorateClubs(); resumePullIfDue();
  });
  ...
  for (const delay of [120, 450, 1000]) setTimeout(() => { syncHud(); decorateClubs(); }, delay);
  const loadClubs = () => ensureClubDb().then(() => decorateClubs()).catch(() => {});
  if (typeof requestIdleCallback === 'function') requestIdleCallback(loadClubs, { timeout: 3500 }); else setTimeout(loadClubs, 1800);
