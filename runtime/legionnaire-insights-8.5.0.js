(function () {
  ...
  // Legionnaire Insights v8.5.0: player and coach careers in one event-driven runtime,
  ...
  function indexLiveCalls(source) {
    if (!source || liveCallQuality.size) return;
    const qualities = [
  ...
  function decorateManagerDomPredictions(record) {
    ...
    for (const card of document.querySelectorAll('.decision button')) {
      ...
      const optionId = managerOptionIdFromCard(card, step);
      if (bands.length < 2 || !optionId) continue;
      const option = { id: optionId, outcomes: bands.map((entry) => ({ probability: entry.probability })) };
